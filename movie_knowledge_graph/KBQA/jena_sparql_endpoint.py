...
class JenaFuseki:
    def __init__(self, endpoint_url='http://localhost:3030/movie/query'):
        """endpoint_url是TDB路径"""
        self.sparql_conn = SPARQLWrapper(endpoint_url)

    def get_sparql_result(self, query):
        self.sparql_conn.setQuery(query)       # 查询
        self.sparql_conn.setReturnFormat(JSON) # 以json的格式返回
        return self.sparql_conn.query().convert() # 将查询结果返回
...
?s :personName '成龙'.?s :hasActedIn ?m.?m :movieTitle ?x
}
limit 1000
    """
